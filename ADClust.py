...
def load_c_dip_file():
    ...
    if os.path.isfile(dip_compiled):
        ...
        raise Exception("C compiled dip file can not be found.\n"
                        ...
                        "Or add the path of dip.so into your LD_LIBRARY_PATH correctly\n")
...
def _adclust_training(X, n_clusters_current, dip_merge_threshold, cluster_loss_weight,ae_weight_loss, centers_cpu, cluster_labels_cpu,
...
def _adclust(X, dip_merge_threshold, cluster_loss_weight, ae_weight_loss, n_clusters_max,
    ...
    cluster_labels_cpu, n_clusters_current, centers_cpu, autoencoder = _adclust_training(X, n_clusters_start,
                                                                                          dip_merge_threshold,
                                                                                          cluster_loss_weight,
                                                                                          ae_weight_loss,
                                                                                          centers_cpu,
...
class ADClust():
    ...
    def fit(self, X):
        labels, n_clusters, centers, autoencoder = _adclust(X, self.dip_merge_threshold,
...
if __name__ == "__main__":
    ...
    cluster_labels, estimated_cluster_numbers=ADClust.fit(data)
    ...
    if not os.path.exists("output"):
        os.mkdir("output")
    pd.DataFrame({"cluster_labels": cluster_labels}).T.to_csv(
        "output/" + args.name + ".csv", header=False)
